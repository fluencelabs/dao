...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  lbpConfigs = lbpConfigs.sort((a, b) => BigNumber.from(a.token).gt(BigNumber.from(b.token)) ? 1 : -1);
  ...
  for (const config of lbpConfigs) {
    const token = IERC20Metadata__factory.connect(config.token, await hre.ethers.getSigner(deployer));
